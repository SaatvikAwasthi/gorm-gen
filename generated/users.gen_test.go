...
func init() {
	InitializeDB()
	err := _gen_test_db.AutoMigrate(&dao.User{})
	if err != nil {
		fmt.Printf("Error: AutoMigrate(&dao.User{}) fail: %s", err)
	}
}
...
func Test_userQuery(t *testing.T) {
	...
	err = _do.Create(&dao.User{})
	if err != nil {
		t.Error("create item in table <users> fail:", err)
	}

	err = _do.Save(&dao.User{})
	if err != nil {
		t.Error("create item in table <users> fail:", err)
	}

	err = _do.CreateInBatches([]*dao.User{{}, {}}, 10)
	if err != nil {
		t.Error("create item in table <users> fail:", err)
	}
	...
	err = _do.Where(primaryKey.IsNotNull()).FindInBatches(&[]*dao.User{}, 10, func(tx gen.Dao, batch int) error { return nil })
	if err != nil {
	...
	_, err = _do.ScanByPage(&dao.User{}, 0, 1)
	if err != nil {
...
var UserFilterWithNameAndEmailTestCase = []TestCase{}

func Test_user_FilterWithNameAndEmail(t *testing.T) {
	user := newUser(_gen_test_db)
	do := user.WithContext(context.Background()).Debug()

	for i, tt := range UserFilterWithNameAndEmailTestCase {
		t.Run("FilterWithNameAndEmail_"+strconv.Itoa(i), func(t *testing.T) {
			res1, res2 := do.FilterWithNameAndEmail(tt.Input.Args[0].(string), tt.Input.Args[1].(string))
			assert(t, "FilterWithNameAndEmail", res1, tt.Expectation.Ret[0])
			assert(t, "FilterWithNameAndEmail", res2, tt.Expectation.Ret[1])
		})
	}
}
